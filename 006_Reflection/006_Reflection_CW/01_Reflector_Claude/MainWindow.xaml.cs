    ...
    public partial class MainWindow : Window
    {
        ...
        private void LoadAssembly(string filePath)
        {
            try
            {
                ...
                statusText.Text = $"Loaded: {Path.GetFileName(filePath)} ({types.Count()} types)";
            }
            catch (Exception ex)
            {
        ...
        private void ShowFieldDetails(FieldInfo field, StringBuilder details)
        {
            ...
            if (field.IsInitOnly) details.Append("readonly ");
            if (field.IsLiteral) details.Append("const ");
